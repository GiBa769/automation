...
```
Error: expect.toBeVisible: Error: strict mode violation: locator('div.bg-panel').filter({ has: locator('h3') }) resolved to 2 elements:
...
Call log:
  ...
  - waiting for locator('div.bg-panel').filter({ has: locator('h3') })
...
```ts
   ...
   9 |     const loginPopup = page.locator('div.bg-panel').filter({ has:page.locator('h3',{Text: 'Đăng nhập' })});
